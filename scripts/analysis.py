...
def main():
    file_mc = TFile.Open("./mc_root_files/LUCAS_TB16_5GeV.root")
    tree_mc = file_mc.LumiCal
    ...
    file_5gev = TFile.Open('./extracted_trees/with_1.4_energy_cut/extracted_5_gev_energy_scan_1.root', 'read')
    tree_5gev = file_5gev.data

    file_4gev = TFile.Open('./extracted_trees/with_1.4_energy_cut/extracted_4_gev_energy_scan_1.root', 'read')
    tree_4gev = file_4gev.data

    file_3gev = TFile.Open('./extracted_trees/with_1.4_energy_cut/extracted_3_gev_energy_scan_1.root', 'read')
    tree_3gev = file_3gev.data

    file_2gev = TFile.Open('./extracted_trees/with_1.4_energy_cut/extracted_2_gev_energy_scan_1.root', 'read')
    tree_2gev = file_2gev.data

    file_1gev = TFile.Open('./extracted_trees/with_1.4_energy_cut/extracted_1_gev_energy_scan_1.root', 'read')
    ...
    tree_mc.Draw('hit_pad-46.58>>h_mc(64, -32., 32.)', "hit_layer == 1 && (hit_sector == 2 || hit_sector == 3) && hit_pad >20")
    ...
    for i, tree in enumerate(trees):
        if i == 0:
            tree.Draw('tr1_hit_pad-cal_cluster_pad[0]>>h_energy{}(64, -32., 32)'.format(i + 1), "", "histosame")
        else:
            tree.Draw('tr1_hit_pad-cal_cluster_pad[0]>>h_energy{}(64, -32., 32)'.format(i + 1), "", "histosame")
